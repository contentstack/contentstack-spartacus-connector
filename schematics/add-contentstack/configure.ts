...
function buildFeatureModule(options: SpartacusContentstackOptions): string {
  const ph = (v: string | undefined, placeholder: string) => (v && v.length ? v : placeholder);
  ...
  const previewToken = ph(options.previewToken, '<PREVIEW_TOKEN>');
 ...
 *
 * The localeMapping, accessControl and livePreview blocks below are scaffolded
 * on purpose (even at their defaults) so they're easy to find and turn on —
 * a missing localeMapping in particular is the usual cause of blank pages.
 */
@NgModule({
    ...
    provideConfig(<ContentstackConfig>{
      contentstack: {
        delivery: {
          ...
          // Live Preview / Visual Builder. \`true\` + a preview token shows draft
          // edits without publishing; leave \`false\` for a normal delivery build.
          livePreview: ${livePreview},
          previewToken: '${previewToken}', // only used when livePreview: true
        },
