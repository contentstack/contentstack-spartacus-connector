...
| `componentTypeMapping` | Block uid → SAP typeCode. **Reserved** — declared on the config but not currently read by the connector. |
...
- **`componentTypeMapping`** — `Record<string, string>`, no default. **Reserved / not currently wired.** It is declared on `ContentstackConfig` to map a Contentstack block uid → Spartacus typeCode for author-named blocks without a `type_code`, but no code in the connector reads it today (typecodes resolve via `TYPECODE_MAP` / `toTypeCode`). Setting it has no effect until it is implemented.
