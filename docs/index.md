...
> [!INFO]
> This is the reference documentation for the connector. The repository's root Markdown files (`README.md`, `GETTING_STARTED.md`, `CONTENT-MODEL.md`, `TROUBLESHOOTING.md`) are the source of truth; these pages organize and expand on them. Contributing to the docs? See the [documentation conventions](schema.md) first.
...
| [Architecture](architecture.md) | The CMS adapter-override chain (`ContentstackCmsPageAdapter` / `ContentstackCmsComponentAdapter`), the normalizer pipeline into `CmsStructureModel`, DI "last-provider-wins" ordering, and the eager `ContentstackCmsFeatureModule` → `ContentstackCmsModule` + `ContentstackLivePreviewModule` composition |
...
| [Content Model](content-model.md) | The page and component content types (`landing_page`, banners, carousels, navigation, …), the slot map (Contentstack field uid → SAP slot name), flat navigation (`*_flat` adjacency list), custom slots, and starter-pack seed guidance |
...
`0.1.0` — framework core validated end-to-end against a real Spartacus app + SAP OCC + Contentstack, plus Live Preview / Visual Editor bindings and the `ng add` schematic installer. The content-model **starter pack ships in the repo** (`import-export/starter-pack/`, imported via `csdx`); a reference storefront and B2B support are tracked separately. See [Status](overview.md#status).
