...
- **`ContentstackLivePreviewModule`** — registers `ContentstackComponentDecorator` on Spartacus's `ComponentDecorator` extension point with `multi: true`. Unlike the CMS adapters (which *replace* the OCC bindings via last-provider-wins), `ComponentDecorator` is a **multi-provider**: the connector's decorator is added **alongside** any others and all of them run, so this is additive, not a substitution.
